...
class Game:
    def __init__(self):
        ...
        # Load npc_data.json
        with open('game/npc_data.json') as f:
        ...
        for i, row in enumerate(overworld):
            for j, column in enumerate(row):
                ...
                if column == "B":
                    Block(self, j, i)
                if column == "P":
                    #Check to see if the player would spawn on an NPC
                    spawn_on_npc = False
                    for npc in self.npc_data:
                        if npc['x'] == j and npc['y'] == i:
                            spawn_on_npc = True
                            break
    ...
    def create_npcs(self):
        for npc_info in self.npc_data:
            NPC(self, npc_info["x"], npc_info["y"], npc_info["dialogue_key"])
        ...
        for event in pygame.event.get():  # every single event that happens in pygame
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if self.player_interacting and self.talking_npc:
                        self.talking_npc.talk()
                    elif self.player_collided and not self.player_interacting:
                        self.talking_npc.talk()
    ...
    def update(self):
        ...
        if not hits:
            self.player_interacting = False #clear interaction if there is no longer a collision
            self.player_collided = False
